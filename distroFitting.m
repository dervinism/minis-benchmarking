...
% meanFittedAmplitudes(8) = 0.1301;
% meanFittedAmplitudes(10) = 0.1984;
% 
...
[L23mean, L23CI] = datamean(meanFittedAmplitudes(~L5)');
[L5mean, L5CI] = datamean(meanFittedAmplitudes(L5)');
[overallMean, overallCI] = datamean(meanFittedAmplitudes');
...
% yLabel = '10-90% rise time (ms)';
% zLabel = 'Count';
% smoothGenericDistro = smoothdata2(genericDistro./nRecs, 'gaussian', 4);
...
% %   ciPlot1Dhisto(simEventAmpDistroInterp, ampBinsInterp, lowerAmpLimitInterp, ...
% %     upperAmpLimitInterp, ampBinsInterp, xLim, fTitle, xLabel, yLabel, figFolder);
% % 
...
% %   plot(ampBinsInterp, simEventAmpDistroInterp, 'b', 'LineWidth',1.25); hold off
% %   xlim(xLim)
...
lowLim2(lowLim2 < 0) = 0;
upLim2 = upLim + CI;
fH = figure; ciplot(lowLim2, upLim2, xBinsCI, 'y', 0.4);
hold on; ciplot(lowLim, upLim, xBinsCI, 'g', 0.3);
plot(xBins, distribution, 'b', 'LineWidth',1.25); hold off
xlim(xLim)
