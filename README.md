...
|csv_MA 	                | *This folder contains ```Mini Analysis``` simulated EPSP detection output tables saved in the CSV format. The CSV files correspond each ABF file in the ```abf``` folder.*|
|csv_pC_raw 	            | *This folder contains Clampfit simulated EPSP detection output tables saved in the CSV format. The CSV files correspond each ABF file in the ```abf_raw``` folder.*|
