function [outputFilename, performance] = evaluatepClamp(baseFilename, realDataDir, noiseFilename, excludedTimesNoise)


%% Load real event data
[csvDir, baseFilenameShort] = fileparts(baseFilename);
realData = load([realDataDir filesep baseFilenameShort '.mat']);
dt = realData.dt;
...
header = {'Trace'; 'Search'; 'Category'; 'State'; 'Event Start Time (ms)'; 'Event End Time (ms)'; 'Baseline (mV)'; 'Peak Amp (mV)';
    ...
    'Area (mV*ms)'; 'Peak-to-Peak Frequency (Hz)'; 'Interevent Interval (ms)'; 'S.D. of Fit'; 'Template'};
peakTimeColumn = find(contains(header, 'Time of Peak (ms)'));


%% Initialise output variables
d = dir([baseFilename '*.csv']);
nRuns = numel(d);
...
for iRun = 1:nRuns
    ...
    filename = [csvDir filesep d(iRun).name];
    opts = detectImportOptions(filename);
    opts.DataLines = [2,Inf];
    opts.VariableTypes(1, 1:end) = {'char'};
    data = readtable(filename, opts);
    dataVarName = data.Properties.VariableNames{peakTimeColumn};
    positivesT = data.(dataVarName); %#ok<*FNDSB>
    ...
    positivesT = positivesT(~isnan(positivesT));
...
outputFilename = [baseFilename '_pClamp.mat'];
